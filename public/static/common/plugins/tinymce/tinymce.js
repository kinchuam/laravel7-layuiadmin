...
layui.define(['jquery'],function (exports) {
    ...
    var settings = {
        ...
        , success: function (res, succFun, failFun) {//上传完成回调
            if (res[this.response.statusName] == this.response.statusCode.ok) {
                succFun(res[this.response.dataName]['file_url']);
            } else {
    ...
    t.render = function (option) {
        ...
        option.plugins = option.plugins ? option.plugins : 'code quickbars print preview searchreplace autolink fullscreen image axupimgs link media codesample table charmap hr advlist lists wordcount imagetools indent2em lineheight';

        option.toolbar = option.toolbar ? option.toolbar : 'code | undo redo | forecolor backcolor bold italic underline strikethrough | indent2em lineheight alignleft aligncenter alignright alignjustify outdent indent | link bullist numlist image axupimgs table codesample | formatselect fontselect fontsizeselect';
        ...
        option.images_upload_handler = function (blobInfo, succFun, failFun) {
            ...
            formData.append('target', option.field||'edit');

            formData.append(option.field||'edit', blobInfo.blob());
